...
**왜 tempId(Redis)와 courseUuid(MySQL)를 분리했는가**<br>
"아직 확정 안 된 코스"와 "영구 저장된 코스"는 조회 방식과 만료 정책이 다릅니다. 하나의 식별자로 묶으면 TTL 로직과 영구 저장 로직이 서로 얽히기 때문에, 역할이 다른 두 식별자로 분리했습니다.

**왜 ApiClient와 Service를 분리했는가**<br>
`@Retryable`은 Spring AOP 프록시 기반으로 동작해서, 같은 클래스 안에서 self-invocation으로 호출하면 재시도가 걸리지 않는 문제를 겪었습니다. 이 경험을 계기로 외부 API 호출 책임(`GeminiApiClient`, `KakaoApiClient`)과 비즈니스 로직(`GeminiService`, `KakaoSearchService`)을 완전히 분리하는 패턴을 세웠습니다. 덕분에 이후 다른 추천 소스가 추가되더라도 같은 패턴으로 확장할 수 있습니다.
 
**AI 응답을 어디까지 신뢰할 것인가**<br>
Google Search 그라운딩을 켰을 때 Gemini가 `responseMimeType`을 강제할 수 없고, 응답 앞에 검색에 사용한 후보 배열이 그대로 echo되어 JSON 파싱이 깨지는 경우를 겪었습니다. 이후로는 "AI 응답은 항상 예측 가능한 형식으로 온다"고 가정하지 않고, 중괄호/대괄호 깊이를 추적하는 파서로 방어적으로 파싱하도록 바꿨습니다.

**지금 단계에서의 한계와 확장 방향**<br>
