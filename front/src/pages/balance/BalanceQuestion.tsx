...
const BalanceQuestion: React.FC<GameScreenProps> = ({
    ...
    return (
        <div className="min-h-screen w-full bg-white flex justify-center">
                        ...
                        <>
                            ...
                            <nav className="px-6 py-4 space-y-5">
                                <button
                                    type="button"
                                    onClick={() => nav('/')}
                                    className="cursor-pointer flex items-center gap-3 text-lg font-semibold"
                                >
                                    🏠  홈
                                </button>
                            </nav>
                ...
                <main className="flex-1 flex flex-col px-6 items-center">
                            ...
                            return (
                                <button
                                    ...
                                    className={`rounded-full transition-all duration-300 cursor-pointer hover:brightness-90 ${
                                        isActive ? 'w-3 h-3 bg-[#FF6B95] scale-110 shadow-sm' : 'w-2.5 h-2.5 bg-[#FFD1E0]'
                                    }`}
                    ...
                    <div className="flex gap-4 w-full mb-10 justify-between mt-4">
                        <div className="w-1/2 aspect-square rounded-[2rem] overflow-hidden shadow-md bg-[#FFF0F5]">
                            <img
                                src={currentData.imgA}
                                alt={`Option A for question ${currentData.id}`}
                                className="w-full h-full object-cover sepia-[0.3] opacity-90 transition-transform duration-500"
                            />
                        </div>
                        <div className="w-1/2 aspect-square rounded-[2rem] overflow-hidden shadow-md bg-[#FFF0F5]">
                            <img
                                src={currentData.imgB}
                                alt={`Option B for question ${currentData.id}`}
                                className="w-full h-full object-cover sepia-[0.3] opacity-90 transition-transform duration-500"
                            />
                        </div>
                    </div>
                    ...
                    <div className="relative mb-10 text-center">
                        <h2 className="text-[1.35rem] font-bold text-black tracking-tight flex items-center justify-center gap-1">
                            {currentData.title || '질문을 불러오는 중...'}
                        </h2>
                    ...
                    <div className="w-full relative flex flex-col gap-6 mb-8">
                        <button
                            onClick={() => onNextQuestion('A')}
                            className="w-full bg-white h-[5.5rem] rounded-[1.5rem] shadow-sm hover:bg-gray-50 active:scale-[0.98] transition-all flex items-center justify-center px-4 border border-white"
                        >
              <span className="cursor-pointer text-gray-800 text-[1.05rem] font-medium tracking-tight whitespace-pre-wrap text-center leading-tight">
                {currentData.optionA || '로딩...'}
              </span>
                        </button>
                        ...
                        <button
                            onClick={() => onNextQuestion('B')}
                            className="w-full bg-white h-[5.5rem] rounded-[1.5rem] shadow-sm hover:bg-gray-50 active:scale-[0.98] transition-all flex items-center justify-center px-4 border border-white"
                        >
              <span className="cursor-pointer text-gray-800 text-[1.05rem] font-medium tracking-tight whitespace-pre-wrap text-center leading-tight">
                {currentData.optionB || '로딩...'}
              </span>
                        </button>
                    </div>
                </main>
          ...
          <span className="text-gray-300 text-xs font-light tracking-[0.2em]">
